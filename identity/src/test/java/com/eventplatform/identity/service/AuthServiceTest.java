...
class AuthServiceTest {
  ...
  void should_issue_access_and_refresh_tokens_on_login() throws Exception {
    ...
    when(jwtTokenProvider.generateToken(11L, user.getRole(), null, user.getEmail()))
        .thenReturn("access-token");

    AuthTokensResponse response =
        authService.login(new LoginRequest("user@example.com", "plain-pass"));
  ...
  void should_revoke_refresh_token_on_logout() {
    ...
    RefreshToken token =
        new RefreshToken(user, sha("refresh-token"), Instant.now().plusSeconds(100));
    when(refreshTokenRepository.findByTokenHash(sha("refresh-token")))
  ...
  void should_issue_new_access_token_when_refresh_token_valid() throws Exception {
    ...
    when(refreshTokenRepository.findByTokenHash(sha("refresh-token")))
        .thenReturn(Optional.of(token));
    when(jwtTokenProvider.generateToken(20L, user.getRole(), null, user.getEmail()))
  ...
  void should_reset_password_when_token_is_latest_unexpired_unconsumed() throws Exception {
    ...
    PasswordResetToken token =
        new PasswordResetToken(user, sha("reset-token"), Instant.now().plusSeconds(100));
    setId(token, 300L);

    when(passwordResetTokenRepository.findByTokenHash(sha("reset-token")))
        .thenReturn(Optional.of(token));
    when(passwordResetTokenRepository.findTopByUserIdOrderByCreatedAtDesc(33L))
        .thenReturn(Optional.of(token));
    when(passwordEncoder.encode("new-pass")).thenReturn("new-hash");

    authService.confirmPasswordReset(
        new PasswordResetConfirmRequest("reset-token", "new-pass", "new-pass"));
