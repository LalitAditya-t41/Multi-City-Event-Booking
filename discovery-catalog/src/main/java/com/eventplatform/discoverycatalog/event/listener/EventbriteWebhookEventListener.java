...
import com.eventplatform.discoverycatalog.exception.MissingWebhookPayloadException;
...
public class EventbriteWebhookEventListener {
    ...
    public void onWebhook(EbWebhookReceivedEvent event) {
        ...
        try {
            eventCatalogRepository.findByEventbriteEventId(eventId).ifPresentOrElse(catalogItem -> {
                webhookStateManager.recordWebhookSuccess(orgId, Instant.now());
                snapshotCache.invalidate(orgId, catalogItem.getCityId());
                eventPublisher.publishEvent(new EventCatalogUpdatedEvent(
                    orgId,
                    catalogItem.getCityId(),
                    catalogItem.getVenueId(),
                    Instant.now()
                ));
            }, () -> {
