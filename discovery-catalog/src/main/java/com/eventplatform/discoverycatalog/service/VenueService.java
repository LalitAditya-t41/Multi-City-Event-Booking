...
public class VenueService implements VenueServiceI {
  ...
  public Venue updateVenue(Long organizationId, Long venueId, UpdateVenueRequest request) {
    ...
    String formattedAddress =
        venueMapper.formatAddress(
            request.addressLine1(), request.addressLine2(), request.country());
    venue.updateDetails(
        request.name(),
        formattedAddress,
        request.zipCode(),
    ...
    try {
      if (venue.getEventbriteVenueId() != null) {
        ebVenueService.updateVenue(
            organizationId, venue.getEventbriteVenueId(), toEbRequest(request));
  ...
  public Venue syncVenue(Long organizationId, Long venueId) {
    ...
    try {
      ...
      if (venue.getEventbriteVenueId() == null) {
        ...
        response =
            ebVenueService.updateVenue(
                organizationId, venue.getEventbriteVenueId(), toEbRequest(venue));
  ...
  public Page<Venue> listFlaggedVenues(int page, int size) {
    return venueRepository.findBySyncStatus(
        VenueSyncStatus.DRIFT_FLAGGED, PageRequest.of(page, size));
  }
