...
public class EventCatalogSyncService {
    ...
    private List<EbEventDto> fetchEvents(Long organizationId, Long venueId) {
        try {
            if (venueId != null) {
                String eventbriteVenueId = venueRepository.findById(venueId)
                    .map(Venue::getEventbriteVenueId)
                    .orElse(null);
    ...
    private void upsertEvent(Long organizationId, Long cityId, EbEventDto ebEvent) {
        ...
        if (!resolvedCityId.equals(cityId)) {
            log.debug("Event city mismatch. eventId={} expectedCityId={} actualCityId={}", ebEvent.id(), cityId, resolvedCityId);
        }
        EventCatalogItem incoming = new EventCatalogItem(
            organizationId,
            resolvedCityId,
            ebEvent.id(),
            ebEvent.name(),
        ...
        eventCatalogRepository.findByEventbriteEventId(ebEvent.id()).ifPresentOrElse(existing -> {
    ...
    private void softDeleteMissing(Long organizationId, Long cityId, Long venueId, Set<String> eventbriteIds) {
        List<EventCatalogItem> existing;
        if (venueId == null) {
            existing = eventCatalogRepository.findByOrganizationIdAndCityIdAndDeletedAtIsNull(organizationId, cityId);
        } else {
            existing = eventCatalogRepository.findByOrganizationIdAndCityIdAndVenueIdAndDeletedAtIsNull(organizationId, cityId, venueId);
        }
        Instant now = Instant.now();
        List<EventCatalogItem> toDelete = existing.stream()
            .filter(item -> !eventbriteIds.contains(item.getEventbriteEventId()))
            .toList();
        toDelete.forEach(item -> item.softDelete(now));
        if (!toDelete.isEmpty()) {
            log.info("Soft-deleted {} missing events for orgId={} cityId={} venueId={}", toDelete.size(), organizationId, cityId, venueId);
        }
    }
}
