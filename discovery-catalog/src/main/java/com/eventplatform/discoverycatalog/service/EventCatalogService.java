...
public class EventCatalogService {
    ...
    public EventCatalogSearchResponse search(Long organizationId, EventCatalogSearchRequest request) {
        ...
        Optional<SnapshotPayload> snapshotOpt = snapshotCache.getSnapshot(organizationId, request.cityId());
        if (snapshotOpt.isPresent()) {
            ...
            Page<EventCatalogItem> page = eventCatalogRepository.findAll(buildSpecification(organizationId, request),
                PageRequest.of(request.page(), request.size()));
            events = page.map(eventCatalogMapper::toResponse).getContent();
            refreshSnapshotCacheFromDb(organizationId, request.cityId());
            PaginationInfo pagination = new PaginationInfo(request.page(), request.size(), page.getTotalElements(), page.getTotalPages());
            return new EventCatalogSearchResponse(events, triggeredRefresh, snapshotTimestamp, source, pagination);
        }
        ...
        return new EventCatalogSearchResponse(pagedEvents, triggeredRefresh, snapshotTimestamp, source, pagination);
    }
    ...
    private boolean triggerRefreshIfStale(Long organizationId, Long cityId) {
        ...
        return webhookConfigRepository.findByOrganizationId(organizationId)
            .map(config -> {
                Instant lastWebhookAt = config.getLastWebhookAt();
                if (lastWebhookAt == null || lastWebhookAt.isBefore(now.minus(WEBHOOK_STALE_THRESHOLD))) {
    ...
    private List<EventCatalogItemResponse> applyFilters(List<EventCatalogItemResponse> events, EventCatalogSearchRequest request) {
        ...
        if (request.q() != null && !request.q().isBlank()) {
            ...
            predicate = predicate.and(item -> item.name() != null && item.name().toLowerCase().contains(qLower));
        }
        if (request.startAfter() != null) {
            predicate = predicate.and(item -> item.startTime() != null && item.startTime().isAfter(request.startAfter()));
        }
        if (request.startBefore() != null) {
            predicate = predicate.and(item -> item.startTime() != null && item.startTime().isBefore(request.startBefore()));
        }
        return events.stream().filter(predicate).toList();
    }

    private Specification<EventCatalogItem> buildSpecification(Long organizationId, EventCatalogSearchRequest request) {
        return (root, query, cb) -> {
            ...
            if (request.q() != null && !request.q().isBlank()) {
                predicates = cb.and(predicates, cb.like(cb.lower(root.get("name")), "%" + request.q().toLowerCase() + "%"));
            }
            if (request.startAfter() != null) {
                predicates = cb.and(predicates, cb.greaterThan(root.get("startTime"), request.startAfter()));
            }
            if (request.startBefore() != null) {
    ...
    private List<EventCatalogItemResponse> paginate(List<EventCatalogItemResponse> events, int page, int size) {
        if (size == 0) {
            return events;
        }
    ...
    private void refreshSnapshotCacheFromDb(Long organizationId, Long cityId) {
        try {
            List<EventCatalogItemResponse> allEvents = eventCatalogRepository
                .findByOrganizationIdAndCityIdAndDeletedAtIsNull(organizationId, cityId)
                .stream()
                .map(eventCatalogMapper::toResponse)
                .toList();
            snapshotCache.putSnapshot(organizationId, cityId, new SnapshotPayload(allEvents, Instant.now()));
        } catch (Exception ex) {
            log.warn("Failed to rebuild snapshot cache from DB. orgId={} cityId={}", organizationId, cityId, ex);
        }
    }
}
