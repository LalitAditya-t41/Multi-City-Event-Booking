...
public class VenueReconciliationJob {
  ...
  public void reconcile() {
    ...
    for (Venue venue : venues) {
      ...
      try {
        EbVenueResponse ebVenue =
            ebVenueService.getVenue(venue.getOrganizationId(), venue.getEventbriteVenueId());
        if (ebVenue == null) {
          venue.markDrift("Eventbrite venue missing");
          eventPublisher.publishEvent(
              new VenueDriftDetectedEvent(
                  venue.getId(), venue.getEventbriteVenueId(), "Eventbrite venue missing"));
          continue;
        }
        ...
        if (expectedAddress != null
            && actualAddress != null
            && !expectedAddress.equalsIgnoreCase(actualAddress)) {
          String drift =
              "EB address changed from '%s' to '%s'".formatted(expectedAddress, actualAddress);
          venue.markDrift(drift);
          eventPublisher.publishEvent(
              new VenueDriftDetectedEvent(venue.getId(), venue.getEventbriteVenueId(), drift));
        }
      } catch (EbIntegrationException ex) {
        log.warn(
            "Venue reconciliation failed. venueId={} ebVenueId={}",
            venue.getId(),
            venue.getEventbriteVenueId(),
            ex);
      } catch (Exception ex) {
        log.warn(
            "Unexpected venue reconciliation error. venueId={} ebVenueId={}",
            venue.getId(),
            venue.getEventbriteVenueId(),
            ex);
      }
