...
class EventCatalogServiceTest {
  ...
  @Mock private EventCatalogRepository eventCatalogRepository;
  ...
  @Mock private EventCatalogSnapshotCache snapshotCache;

  @Mock private EventCatalogMapper eventCatalogMapper;

  @Mock private EventCatalogRefreshService refreshService;

  @Mock private EventCatalogMetrics metrics;

  @InjectMocks private EventCatalogService service;
  ...
  void should_return_db_results_when_cache_miss() {
    ...
    EventCatalogSearchRequest request =
        new EventCatalogSearchRequest(cityId, null, null, null, null, null, 0, 20);
    ...
    EventCatalogItem item =
        new EventCatalogItem(
            orgId, cityId, "eb_1", "Show", Instant.now(), Instant.now().plusSeconds(3600));
    EventCatalogItemResponse response =
        new EventCatalogItemResponse(
            1L,
            cityId,
            null,
            "eb_1",
    ...
    when(eventCatalogRepository.findAll(
            org.mockito.ArgumentMatchers.<Specification<EventCatalogItem>>any(),
