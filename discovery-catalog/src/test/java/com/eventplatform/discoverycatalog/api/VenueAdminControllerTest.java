...
@Import({
  ...
  VenueAdminControllerTest.MockBeans.class
})
...
class VenueAdminControllerTest {
  ...
  static class MockBeans {
    ...
    VenueServiceI venueService() {
      return mock(VenueServiceI.class);
    }

    @Bean
    VenueMapper venueMapper() {
      return mock(VenueMapper.class);
    }

    @Bean
    CityCatalogService cityCatalogService() {
      return mock(CityCatalogService.class);
    }

    @Bean
    EventCatalogService eventCatalogService() {
      return mock(EventCatalogService.class);
    }

    @Bean
    VenueCatalogService venueCatalogService() {
      return mock(VenueCatalogService.class);
    }
  ...
  @Autowired private VenueServiceI venueService;
  ...
  void should_return_400_when_name_blank() throws Exception {
    CreateVenueRequest request =
        new CreateVenueRequest(
            "", "Addr1", null, 3L, "IN", "123", "12.9", "77.6", 100, SeatingMode.GA);
  ...
  void should_return_200_when_venue_updated() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/admin/catalog/venues/10")
                .queryParam("orgId", "1")
                .contentType(MediaType.APPLICATION_JSON)
  ...
  private CreateVenueRequest baseCreateRequest() {
    return new CreateVenueRequest(
        "Venue", "Addr1", null, 3L, "IN", "123", "12.9", "77.6", 100, SeatingMode.GA);
  }

  private UpdateVenueRequest baseUpdateRequest() {
    return new UpdateVenueRequest(
        "Venue", "Addr1", null, 3L, "IN", "123", "12.9", "77.6", 120, SeatingMode.GA);
  }
