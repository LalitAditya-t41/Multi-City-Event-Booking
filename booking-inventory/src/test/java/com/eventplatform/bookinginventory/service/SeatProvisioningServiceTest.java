...
class SeatProvisioningServiceTest {
    ...
    @Mock
    private CatalogSeatLayoutClient catalogSeatLayoutClient;
    @Mock
    private GaInventoryService gaInventoryService;

    @InjectMocks
    private SeatProvisioningService seatProvisioningService;
    ...
    void should_bulk_insert_seats_from_venue_seat_layout_on_slot_activated() {
        ...
        when(catalogSeatLayoutClient.getSeatLayout(20L)).thenReturn(new CatalogSeatLayoutResponse(
            20L,
            1,
            List.of(new CatalogSeatResponse(1L, "S1", "A", "A1", "VIP", false))
        ));
