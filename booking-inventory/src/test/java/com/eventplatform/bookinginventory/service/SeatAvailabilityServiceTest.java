...
class SeatAvailabilityServiceTest {
  ...
  void should_return_available_seats_excluding_non_expired_locks() {
    SlotSummaryDto slot =
        new SlotSummaryDto(11L, "ACTIVE", "eb-11", SeatingMode.RESERVED, 1L, 2L, 3L, null);
    PricingTierDto tier = tier();
    Seat seat = seat(100L, SeatLockState.AVAILABLE);
    AvailableSeatResponse mapped =
    ...
    when(seatRepository.findAvailableForSlot(
            org.mockito.ArgumentMatchers.eq(11L), org.mockito.ArgumentMatchers.any(Instant.class)))
  ...
  void should_include_expired_soft_locked_seats_in_available_response() {
    SlotSummaryDto slot =
        new SlotSummaryDto(12L, "ACTIVE", "eb-12", SeatingMode.RESERVED, 1L, 2L, 3L, null);
    PricingTierDto tier = tier();
    ...
    when(seatRepository.findAvailableForSlot(
            org.mockito.ArgumentMatchers.eq(12L), org.mockito.ArgumentMatchers.any(Instant.class)))
        .thenReturn(List.of(seat));
    when(seatMapper.toAvailableSeatResponse(seat, "VIP", new BigDecimal("500.00"), "INR"))
        .thenReturn(
            new AvailableSeatResponse(
  ...
  void should_return_ga_tier_availability_from_redis_counter() {
    SlotSummaryDto slot =
        new SlotSummaryDto(13L, "ACTIVE", "eb-13", SeatingMode.GA, 1L, 2L, 3L, null);
    PricingTierDto tier = tier();
  ...
  void should_fallback_to_db_when_redis_counter_missing() {
    SlotSummaryDto slot =
        new SlotSummaryDto(14L, "ACTIVE", "eb-14", SeatingMode.GA, 1L, 2L, 3L, null);
    PricingTierDto tier = tier();
    ...
    when(gaInventoryClaimRepository.countByShowSlotIdAndPricingTierIdAndLockState(
            14L, 10L, SeatLockState.CONFIRMED))
        .thenReturn(8L);
    ...
    verify(gaInventoryClaimRepository)
        .countByShowSlotIdAndPricingTierIdAndLockState(14L, 10L, SeatLockState.CONFIRMED);
  }

  @Test
  void should_mark_tier_blocked_when_redis_block_flag_set() {
    SlotSummaryDto slot =
        new SlotSummaryDto(15L, "ACTIVE", "eb-15", SeatingMode.GA, 1L, 2L, 3L, null);
    PricingTierDto tier = tier();
