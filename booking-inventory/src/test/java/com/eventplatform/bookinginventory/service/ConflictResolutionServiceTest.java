...
class ConflictResolutionServiceTest {
  ...
  void should_return_up_to_5_same_section_alternatives() {
    ...
    when(seatRepository.findAlternativesSameSection(100L, "S1", 10L))
        .thenReturn(
            List.of(
                seat(2L, "A2", "S1"),
                seat(3L, "A3", "S1"),
  ...
  void should_return_up_to_3_adjacent_section_alternatives_when_same_section_insufficient() {
    Seat requested = seat(1L, "A1", "S1");
    when(seatRepository.findAlternativesSameSection(100L, "S1", 10L))
        .thenReturn(List.of(seat(2L, "A2", "S1")));
    when(seatRepository.findAlternativesAdjacentSection(100L, "S1", 10L))
        .thenReturn(
            List.of(
                seat(8L, "B1", "S2"),
                seat(9L, "B2", "S2"),
