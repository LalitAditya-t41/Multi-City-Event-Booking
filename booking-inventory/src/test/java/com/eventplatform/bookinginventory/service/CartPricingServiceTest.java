...
class CartPricingServiceTest {
  ...
  @Test
  void getSlotPricingCached_should_return_cached_list_when_redis_hit_and_not_call_reader()
      throws Exception {
  ...
  void getSlotPricingCached_should_call_reader_and_write_cache_on_redis_miss() {
    ...
    verify(valueOperations)
        .set(eq("pricing:slot:11"), any(String.class), eq(Duration.ofMinutes(10)));
  }
  ...
  void getSlotPricingCached_should_not_propagate_exception_when_redis_write_fails() {
    ...
    doThrow(new RedisConnectionFailureException("write down"))
        ...
        .set(eq("pricing:slot:13"), any(String.class), eq(Duration.ofMinutes(10)));
  ...
  private CartItem item(Long itemId, Long tierId, String price) {
    CartItem item =
        new CartItem(
            itemId, null, tierId, "tc_" + tierId, new Money(new BigDecimal(price), "INR"), 1);
