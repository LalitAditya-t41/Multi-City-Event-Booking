...
@ContextConfiguration(classes = com.eventplatform.bookinginventory.BookingInventoryTestApplication.class)
@Import({GlobalExceptionHandler.class, SecurityConfig.class, BookingInventoryControllerTest.MockBeans.class})
class BookingInventoryControllerTest {
    ...
    void should_return_200_with_seat_map_for_active_reserved_slot() throws Exception {
        when(seatAvailabilityService.getAvailability(10L)).thenReturn(new AvailableSeatsEnvelopeResponse(
            10L,
            SeatingMode.RESERVED,
            List.of(new AvailableSeatResponse(
                101L,
                "A1",
                "A",
                "S1",
        ...
        mockMvc.perform(get("/api/v1/booking/shows/10/available-seats")
    ...
    void should_return_200_with_ga_tier_availability_for_active_ga_slot() throws Exception {
        when(seatAvailabilityService.getAvailability(11L)).thenReturn(new AvailableSeatsEnvelopeResponse(
            11L,
            SeatingMode.GA,
            List.of(),
            List.of(new GaTierAvailabilityResponse(
                1L,
                "GA",
                100,
                88L,
                new Money(new BigDecimal("300.00"), "INR"),
                false
            ))
        ));

        mockMvc.perform(get("/api/v1/booking/shows/11/available-seats")
                .with(authentication(userAuth())))
            .andExpect(status().isOk())
    ...
    void should_return_503_with_EB_EVENT_NOT_SYNCED_when_eb_event_id_null() throws Exception {
        when(seatAvailabilityService.getAvailability(12L)).thenThrow(new BaseException(
        ...
        mockMvc.perform(get("/api/v1/booking/shows/12/available-seats")
                .with(authentication(userAuth())))
    ...
    void should_return_409_with_alternatives_when_seat_unavailable() throws Exception {
        when(cartService.addSeat(eq(1L), eq(44L), eq("USER"), any(AddSeatRequest.class)))
            .thenThrow(new SeatUnavailableException(200L, new SeatAlternativesResponse(
                200L,
                List.of(new SeatSuggestionResponse(201L, "A2", "S1", 1L)),
                List.of()
            )));

        mockMvc.perform(post("/api/v1/booking/cart/add-seat")
                .with(authentication(userAuth()))
    ...
    void should_return_409_with_existing_cart_id_when_duplicate_cart_attempted() throws Exception {
        when(cartService.addSeat(eq(1L), eq(44L), eq("USER"), any(AddSeatRequest.class)))
            .thenThrow(new BaseException("Pending cart already exists", "CART_ALREADY_EXISTS", HttpStatus.CONFLICT,
                Map.of("cartId", 500L)) {
            });
