...
public class SlotValidationService {
    ...
    public SlotSummaryDto requireActiveAndSynced(Long slotId) {
        ...
        if (slot.ebEventId() == null || slot.ebEventId().isBlank()) {
            throw new BaseException("Eventbrite event id is missing for slot", "EB_EVENT_NOT_SYNCED", HttpStatus.SERVICE_UNAVAILABLE,
                Map.of("slotId", slotId)) {
            };
        }
        if (!"ACTIVE".equalsIgnoreCase(slot.status())) {
            throw new BaseException("Slot is not active", "SLOT_NOT_ACTIVE", HttpStatus.CONFLICT, Map.of("currentStatus", slot.status())) {
            };
        }
        return slot;
    }
    ...
    public void ensureTierSynced(Long tierId, String ebTicketClassId) {
        if (ebTicketClassId == null || ebTicketClassId.isBlank()) {
            throw new BaseException("Tier not synced to Eventbrite", "TIER_NOT_SYNCED", HttpStatus.SERVICE_UNAVAILABLE,
