...
public class CartPricingService {
  ...
  public record CartPricingResult(
      Money subtotal, Money discount, Money total, Map<Long, Money> itemDiscounts) {}
    ...
    try {
      ...
      if (cachedJson != null && !cachedJson.isBlank()) {
        return redisObjectMapper.readValue(
            cachedJson, new TypeReference<List<PricingTierDto>>() {});
      }
  ...
  public CartPricingResult recompute(Cart cart, List<CartItem> items) {
    ...
    BigDecimal subtotalAmount =
        items.stream().map(this::lineAmount).reduce(BigDecimal.ZERO, BigDecimal::add);

    Map<Long, List<CartItem>> byTier =
        items.stream().collect(Collectors.groupingBy(CartItem::getPricingTierId));
    Map<Long, GroupDiscountRule> rulesByTier =
        groupDiscountRuleRepository.findByShowSlotId(cart.getShowSlotId()).stream()
            .collect(
                Collectors.toMap(
                    GroupDiscountRule::getPricingTierId, Function.identity(), (a, b) -> a));
    ...
    for (Map.Entry<Long, List<CartItem>> entry : byTier.entrySet()) {
      ...
      if (rule == null
          || rule.getGroupDiscountThreshold() == null
          || rule.getGroupDiscountPercent() == null) {
        tierItems.forEach(
            item -> itemDiscounts.put(item.getId(), new Money(BigDecimal.ZERO, currency)));
        continue;
      }
      ...
      BigDecimal tierSubtotal =
          tierItems.stream().map(this::lineAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    Money total =
        new Money(
            subtotal.amount().subtract(discount.amount()).setScale(2, RoundingMode.HALF_UP),
            currency);
