...
public class SeatAvailabilityService {
    ...
    public AvailableSeatsEnvelopeResponse getAvailability(Long slotId) {
        ...
        Map<Long, PricingTierDto> tierMap = tiers.stream().collect(Collectors.toMap(PricingTierDto::tierId, Function.identity()));

        if (slot.seatingMode() == SeatingMode.RESERVED) {
            List<AvailableSeatResponse> seats = seatRepository.findAvailableForSlot(slotId, java.time.Instant.now())
                .stream()
                .map(seat -> {
        ...
        List<GaTierAvailabilityResponse> gaTiers = tiers.stream()
            .map(tier -> {
                Long available = gaInventoryRedisService.getCurrent(slotId, tier.tierId());
                if (available == null) {
                    long confirmed = gaInventoryClaimRepository.countByShowSlotIdAndPricingTierIdAndLockState(slotId, tier.tierId(), SeatLockState.CONFIRMED);
                    available = Math.max(0, tier.quota() - confirmed);
                }
                boolean blocked = Boolean.TRUE.equals(stringRedisTemplate.hasKey("tier:blocked:" + slotId + ":" + tier.tierId()));
