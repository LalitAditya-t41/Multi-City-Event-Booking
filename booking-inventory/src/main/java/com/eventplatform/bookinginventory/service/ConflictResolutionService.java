...
public class ConflictResolutionService {
  ...
  public SeatAlternativesResponse alternativesFor(Seat requested) {
    List<SeatSuggestionResponse> sameSection =
        seatRepository
            .findAlternativesSameSection(
                requested.getShowSlotId(), requested.getSection(), requested.getPricingTierId())
            .stream()
            .limit(5)
            .map(this::toSuggestion)
            .toList();

    List<SeatSuggestionResponse> adjacent =
        seatRepository
            .findAlternativesAdjacentSection(
                requested.getShowSlotId(), requested.getSection(), requested.getPricingTierId())
            .stream()
            .limit(3)
            .map(this::toSuggestion)
  ...
  private SeatSuggestionResponse toSuggestion(Seat seat) {
    return new SeatSuggestionResponse(
        seat.getId(), seat.getSeatNumber(), seat.getSection(), seat.getPricingTierId());
  }
}
