...
public class CartService {
    ...
    public CartResponse addSeat(Long userId, Long orgId, String role, AddSeatRequest request) {
        ...
        Map<Long, PricingTierDto> tierById = tiers.stream().collect(Collectors.toMap(PricingTierDto::tierId, Function.identity()));
    ...
    public CartResponse removeItem(Long userId, Long itemId) {
        ...
        if (item.getSeatId() != null) {
            Seat seat = seatRepository.findByIdWithLock(item.getSeatId())
            ...
            if (seat.getLockState() == SeatLockState.HARD_LOCKED
                ...
                throw new BaseException("Seat removal not allowed", "SEAT_REMOVAL_NOT_ALLOWED", HttpStatus.CONFLICT,
            ...
            writeAudit(seat, userId, previous, SeatLockState.AVAILABLE, SeatLockEvent.RELEASE, LockReleaseReason.USER_REMOVED.name(), null);
        } else if (item.getGaClaimId() != null) {
            gaInventoryClaimRepository.findById(item.getGaClaimId()).ifPresent(claim -> {
                gaInventoryRedisService.restore(claim.getShowSlotId(), claim.getPricingTierId(), claim.getQuantity());
    ...
    public CartResponse confirm(Long userId, Long orgId, String userEmail, ConfirmCartRequest request) {
        ...
        Set<Long> expiredSeatIds = cart.getItems().stream()
        ...
        Map<String, Long> ticketClassToTier = tierById.values().stream()
            .filter(t -> t.ebTicketClassId() != null)
            .collect(Collectors.toMap(PricingTierDto::ebTicketClassId, PricingTierDto::tierId, (a, b) -> a));

        for (String ticketClassId : cart.getItems().stream().map(CartItem::getEbTicketClassId).collect(Collectors.toSet())) {
            ...
            try {
                ...
                throw new BaseException("Eventbrite unavailable", "EB_UNAVAILABLE", HttpStatus.SERVICE_UNAVAILABLE,
            ...
            if (ticketClass != null && "SOLD_OUT".equalsIgnoreCase(ticketClass.onSaleStatus())) {
                ...
                List<Map<String, Object>> alternativeTiers = tierById.values().stream()
        ...
        for (CartItem item : cart.getItems()) {
            if (item.getSeatId() != null) {
                ...
                try {
                    ...
                    seatRepository.save(seat);
                    writeAudit(seat, userId, previous, SeatLockState.HARD_LOCKED, SeatLockEvent.CONFIRM, null, null);
        ...
        for (CartItem item : new ArrayList<>(cart.getItems())) {
            if (item.getSeatId() != null) {
                ...
                if (seat != null && seat.getLockState() != SeatLockState.CONFIRMED) {
                    ...
                    seatRepository.save(seat);
                    seatLockRedisService.release(seat.getId(), userId);
                    writeAudit(seat, userId, previous, SeatLockState.AVAILABLE, SeatLockEvent.RELEASE, LockReleaseReason.CART_ABANDONED.name(), null);
                }
            } else if (item.getGaClaimId() != null) {
                gaInventoryClaimRepository.findById(item.getGaClaimId()).ifPresent(claim -> {
                    gaInventoryRedisService.restore(claim.getShowSlotId(), claim.getPricingTierId(), claim.getQuantity());
    ...
    public void onPaymentFailed(Long cartId, List<Long> seatIds, Long userId) {
        ...
        for (Long seatId : seatIds) {
            Seat seat = seatRepository.findByIdWithLock(seatId).orElse(null);
            if (seat == null || seat.getLockState() == SeatLockState.AVAILABLE || seat.getLockState() == SeatLockState.CONFIRMED) {
                continue;
            }
            SeatLockState previous = seat.getLockState();
            ...
            seatRepository.save(seat);
            seatLockRedisService.release(seatId, userId);
            writeAudit(seat, userId, previous, SeatLockState.AVAILABLE, SeatLockEvent.RELEASE, LockReleaseReason.PAYMENT_FAILED.name(), null);
        }
        cart.abandon();
        cartRepository.save(cart);
    }

    private Cart getOrCreatePendingCart(Long userId, SlotSummaryDto slot, Long orgId, String currency) {
        return cartRepository.findByUserIdAndShowSlotIdAndStatus(userId, slot.slotId(), CartStatus.PENDING)
            .orElseGet(() -> {
                try {
                    Cart created = new Cart(userId, slot.slotId(), orgId, slot.seatingMode(), SOFT_LOCK_TTL, currency);
                    return cartRepository.save(created);
                } catch (DataIntegrityViolationException ex) {
                    return cartRepository.findByUserIdAndShowSlotIdAndStatus(userId, slot.slotId(), CartStatus.PENDING)
                        .orElseThrow(() -> new BusinessRuleException("Pending cart already exists", "CART_ALREADY_EXISTS"));
                }
            });
    }
    ...
    private void addReservedSeat(
        ...
        if (!Objects.equals(seat.getShowSlotId(), request.slotId()) || !Objects.equals(seat.getPricingTierId(), request.tierId())) {
        ...
        if (result == AcquireResult.CONFLICT) {
            throw new SeatUnavailableException(seat.getId(), conflictResolutionService.alternativesFor(seat));
        }
        ...
        try {
            if (result == AcquireResult.ACQUIRED) {
                ...
                seatRepository.save(seat);
                writeAudit(seat, userId, previous, SeatLockState.SOFT_LOCKED, SeatLockEvent.SELECT, null, null);
            }
    ...
    private void addGaTier(Long userId, Cart cart, AddSeatRequest request, PricingTierDto tier) {
        String blocked = stringRedisTemplate.opsForValue().get("tier:blocked:" + request.slotId() + ":" + request.tierId());
        if (blocked != null) {
            throw new TierBlockedException(request.tierId());
        }

        long claimResult = gaInventoryRedisService.claim(request.slotId(), request.tierId(), request.quantity());
        if (claimResult == -2) {
            throw new BaseException("GA counter not initialized", "GA_COUNTER_NOT_INITIALIZED", HttpStatus.SERVICE_UNAVAILABLE) {
            };
        }
        if (claimResult < 0) {
        ...
        GaInventoryClaim claim = new GaInventoryClaim(
            request.slotId(),
            request.tierId(),
            userId,
            cart.getId(),
    ...
    private CartResponse buildCartResponse(Cart cart, Map<Long, PricingTierDto> tierById) {
        ...
        Map<Long, String> tierNames = tierById.values().stream()
            .collect(Collectors.toMap(PricingTierDto::tierId, PricingTierDto::tierName, (a, b) -> a));
        ...
        if (!seatIds.isEmpty()) {
            seatRepository.findAllById(seatIds).forEach(seat -> seatNumbers.put(seat.getId(), seat.getSeatNumber()));
        }
    ...
    private void assertOrgConnected(Long orgId) {
        try {
            ...
            throw new BaseException("Organization is not connected to Eventbrite", "ORG_NOT_CONNECTED", HttpStatus.SERVICE_UNAVAILABLE) {
            };
        }
    }
    ...
    private void releaseTierLocks(Cart cart, Long userId, Long soldOutTierId) {
        for (CartItem item : cart.getItems()) {
            ...
            if (item.getSeatId() != null) {
                seatRepository.findByIdWithLock(item.getSeatId()).ifPresent(seat -> {
                    if (seat.getLockState() == SeatLockState.SOFT_LOCKED) {
                        SeatLockState previous = seat.getLockState();
                        seat.release(LockReleaseReason.TIER_SOLD_OUT);
                        seatRepository.save(seat);
                        seatLockRedisService.release(seat.getId(), userId);
                        writeAudit(seat, userId, previous, SeatLockState.AVAILABLE, SeatLockEvent.RELEASE, LockReleaseReason.TIER_SOLD_OUT.name(), null);
                    }
                });
            }
