...
public class SeatLockStateMachineConfig {
    ...
    private final Map<SeatLockState, Map<SeatLockEvent, SeatLockState>> transitions = new EnumMap<>(SeatLockState.class);

    public SeatLockStateMachineConfig() {
        transitions.put(SeatLockState.AVAILABLE, Map.of(SeatLockEvent.SELECT, SeatLockState.SOFT_LOCKED));
        transitions.put(SeatLockState.SOFT_LOCKED, Map.of(
