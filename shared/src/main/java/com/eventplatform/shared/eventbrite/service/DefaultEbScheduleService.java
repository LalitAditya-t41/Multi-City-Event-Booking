...
import com.eventplatform.shared.eventbrite.dto.response.EbScheduleOccurrence;
...
public class DefaultEbScheduleService implements EbScheduleService {
    ...
    /**
     * Raw response from the mock schedule endpoint.
     * Mock returns: {@code {id, event_id, occurrence_duration, recurrence_rule}}
     * No occurrences list is returned by the mock — {@code occurrences} will be null.
     */
    ...
    public EbScheduleResponse createSchedule(Long organizationId, String eventId, String recurrenceRule) {
        try {
            ...
            ScheduleRawResponse raw = client.post()
                .uri("/v3/events/{eventId}/schedules/", eventId)
                .body(Map.of("schedule", Map.of(
                    "event_id", eventId,
                    "recurrence_rule", recurrenceRule
                )))
                .retrieve()
                .onStatus(HttpStatusCode::isError, (req, resp) -> {
                    throw new EbIntegrationException("EB createSchedule failed: " + resp.getStatusCode());
                })
