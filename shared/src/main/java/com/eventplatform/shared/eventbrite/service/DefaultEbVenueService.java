 ...
 *
 * <p><b>Mock limitation</b>: The mock server ({@code localhost:8888}) does not expose a
 * {@code POST /v3/venues/} creation endpoint — venues are only pre-seeded via
 * {@code POST /mock/reset}. For local testing, create the venue through the mock admin
 * before calling {@link #createVenue}; the seeded venue id ({@code "venue_1"}) can be
 * used directly as the EB venue ID in the local DB.
 *
 * <p>The real Eventbrite v3 API exposes {@code POST /v3/venues/} for venue creation;
 * the implementation below targets that endpoint and will work against production EB.
 */
@Service
