    ...
    public List<EbTicketClassResponse> createTicketClasses(String eventId, List<EbTicketClassRequest> ticketClasses) {
        try {
            ...
            for (EbTicketClassRequest tc : ticketClasses) {
                EbTicketClassResponse resp = client.post()
                    ...
                    .onStatus(HttpStatusCode::isError, (req, resp2) -> {
                        throw new EbIntegrationException("EB createTicketClass failed: " + resp2.getStatusCode());
                    })
    ...
    public List<EbInventoryTierResponse> createInventoryTiers(String eventId, List<EbInventoryTierRequest> tiers) {
        try {
            ...
            for (EbInventoryTierRequest tier : tiers) {
                ...
                EbInventoryTierResponse resp = client.post()
                    .uri("/v3/events/{eventId}/ticket_classes/", eventId)
                    .body(Map.of("ticket_class", Map.of(
                        "name", tier.name(),
                        "quantity_total", tier.quantity()
                    )))
                    .retrieve()
                    .onStatus(HttpStatusCode::isError, (req, resp2) -> {
                        throw new EbIntegrationException("EB createInventoryTier failed: " + resp2.getStatusCode());
                    })
    ...
    public String copySeatMap(String eventId, String sourceSeatMapId) {
        try {
            ...
            var resp = client.post()
                ...
                .retrieve()
                .onStatus(HttpStatusCode::isError, (req, resp2) -> {
                    throw new EbIntegrationException("EB copySeatMap failed: " + resp2.getStatusCode());
                })
                .body(Map.class);
