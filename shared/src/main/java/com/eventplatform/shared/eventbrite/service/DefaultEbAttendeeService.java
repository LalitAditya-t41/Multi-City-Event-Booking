...
public class DefaultEbAttendeeService implements EbAttendeeService {
    ...
    private record AttendeeListResponse(@JsonProperty("attendees") List<EbAttendeeResponse> attendees) {
    }

    @Override
    public List<EbAttendeeResponse> getAttendeesByEvent(String orgToken, String eventId) {
        try {
            AttendeeListResponse response = client.get()
                ...
                .onStatus(HttpStatusCode::isError, (req, resp2) -> {
                    throw new EbIntegrationException("EB getAttendeesByEvent failed: " + resp2.getStatusCode());
                })
