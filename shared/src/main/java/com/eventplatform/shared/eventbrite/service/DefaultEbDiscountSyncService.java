...
public class DefaultEbDiscountSyncService implements EbDiscountSyncService {
    ...
    private record DiscountListResponse(@JsonProperty("discounts") List<EbDiscountResponse> discounts) {}
        ...
        try {
            return client.post()
                ...
                .onStatus(HttpStatusCode::isError, (req, resp) -> {
                    throw new EbIntegrationException("EB createDiscount failed: " + resp.getStatusCode());
                })
    ...
    public List<EbDiscountResponse> listDiscounts(String orgId) {
        try {
            DiscountListResponse response = client.get()
                .uri("/v3/organizations/{orgId}/discounts/", orgId)
                .retrieve()
                .onStatus(HttpStatusCode::isError, (req, resp) -> {
                    throw new EbIntegrationException("EB listDiscounts failed: " + resp.getStatusCode());
                })
    ...
    public void deleteDiscount(String discountId) {
        try {
            client.delete()
                ...
                .retrieve()
                .onStatus(HttpStatusCode::isError, (req, resp) -> {
                    throw new EbIntegrationException("EB deleteDiscount failed: " + resp.getStatusCode());
                })
                .toBodilessEntity();
        } catch (EbIntegrationException ex) {
            throw ex;
