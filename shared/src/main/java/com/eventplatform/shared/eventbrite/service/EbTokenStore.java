...
public class EbTokenStore {
  ...
  public EbTokenStore(
      OrganizationAuthRepository repository, EbOAuthClient oauthClient, TokenCipher tokenCipher) {
  ...
  public String getAccessToken(Long orgId) {
    OrganizationAuth auth =
        repository
            .findByOrganizationId(orgId)
            .orElseThrow(
                () -> new EbAuthException("No Eventbrite auth configured for org: " + orgId));
