 ...
 *
 * <p>Note: Stripe's internal refund status is read-only via their API. There is no way to "submit"
 * a refund via a webhook; all refund creation goes through this facade.
 */
@Service
public class StripeRefundService {
  ...
  public StripeRefundResponse createRefund(StripeRefundRequest request) {
    try {
      RefundCreateParams.Builder builder =
          RefundCreateParams.builder().setPaymentIntent(request.paymentIntentId());

      if (request.amount() != null) {
        builder.setAmount(request.amount());
      }
      if (request.reason() != null) {
        builder.setReason(RefundCreateParams.Reason.valueOf(request.reason().toUpperCase()));
      }
  ...
  private StripeRefundResponse toResponse(Refund refund) {
    return new StripeRefundResponse(
        refund.getId(), refund.getStatus(), refund.getAmount(), refund.getCurrency());
  }
}
