...
class EventCancellationRefundServiceTest {
    ...
    @Mock
    private EventCancellationRefundAuditRepository eventCancellationRefundAuditRepository;
    @Mock
    ...
    void setUp() {
        ...
        eventCancellationRefundService = new EventCancellationRefundService(
            bookingRepository,
    ...
    void should_continue_processing_remaining_bookings_when_one_booking_refund_fails() {
        ...
        when(eventCancellationRefundAuditRepository.findBySlotIdAndBookingId(77L, 1001L)).thenReturn(Optional.empty());
        when(eventCancellationRefundAuditRepository.findBySlotIdAndBookingId(77L, 1002L)).thenReturn(Optional.empty());
        ...
        when(cancellationService.cancelForEventCancellation(1002L)).thenThrow(new RuntimeException("downstream-failure"));
        ...
        ArgumentCaptor<EventCancellationRefundAudit> captor = ArgumentCaptor.forClass(EventCancellationRefundAudit.class);
        verify(eventCancellationRefundAuditRepository, org.mockito.Mockito.atLeast(4)).save(captor.capture());

        List<EventCancellationRefundAudit> saved = captor.getAllValues();
        assertThat(saved.stream().anyMatch(audit -> audit.getBookingId().equals(1001L) && audit.getStatus() == EventCancellationAuditStatus.SUCCEEDED))
            .isTrue();
        assertThat(saved.stream().anyMatch(audit -> audit.getBookingId().equals(1002L) && audit.getStatus() == EventCancellationAuditStatus.FAILED))
            .isTrue();
    }
