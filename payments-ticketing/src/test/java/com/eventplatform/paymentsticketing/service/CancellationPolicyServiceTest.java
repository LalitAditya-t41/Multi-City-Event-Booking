...
class CancellationPolicyServiceTest {
  ...
  @Mock private CancellationPolicyMapper cancellationPolicyMapper;
  ...
  void setUp() {
    cancellationPolicyService =
        new CancellationPolicyService(cancellationPolicyRepository, cancellationPolicyMapper);
  }

  @Test
  void should_resolve_org_policy_when_org_specific_policy_exists() {
    // Arrange
    CancellationPolicy orgPolicy =
        policy(
            88L,
    ...
    verify(cancellationPolicyRepository, never())
        .findByScope(CancellationPolicyScope.SYSTEM_DEFAULT);
  }
  ...
  void should_resolve_system_default_policy_when_org_policy_not_found() {
    ...
    CancellationPolicy defaultPolicy =
        policy(
            null,
            CancellationPolicyScope.SYSTEM_DEFAULT,
            List.of(
                new CancellationPolicyTier(72, 100, 1),
                new CancellationPolicyTier(24, 50, 2),
                new CancellationPolicyTier(null, 0, 3)));

    when(cancellationPolicyRepository.findByOrgId(99L)).thenReturn(Optional.empty());
    when(cancellationPolicyRepository.findByScope(CancellationPolicyScope.SYSTEM_DEFAULT))
  ...
  @Test
  void
      should_compute_refund_amount_with_floor_rounding_when_policy_percent_has_fractional_result() {
    // Arrange
    CancellationPolicy defaultPolicy =
        policy(
            null,
  ...
  void should_return_effective_policy_response_when_fetching_effective_policy() {
    ...
    CancellationPolicy defaultPolicy =
        policy(
            ...
            CancellationPolicyScope.SYSTEM_DEFAULT,
            List.of(new CancellationPolicyTier(24, 50, 1), new CancellationPolicyTier(null, 0, 2)));
    CancellationPolicyResponse mapped =
        new CancellationPolicyResponse(
            1L, null, CancellationPolicyScope.SYSTEM_DEFAULT, null, List.of());
  ...
  private Booking booking(Long orgId, Instant slotStartTime) {
    return new Booking("BK-TEST-1", 1L, 2L, 3L, 4L, orgId, slotStartTime, 1000L, "inr");
  }

  private CancellationPolicy policy(
      Long orgId, CancellationPolicyScope scope, List<CancellationPolicyTier> tiers) {
