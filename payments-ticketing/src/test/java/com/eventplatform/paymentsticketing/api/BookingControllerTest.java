...
class BookingControllerTest {
  ...
  void should_return_200_with_paginated_bookings_for_authenticated_user() throws Exception {
    when(bookingService.listBookings(any(), any()))
        .thenReturn(
            new PageImpl<>(
                List.of(
    ...
    mockMvc
        .perform(get("/api/v1/bookings").with(authentication(userAuthentication())))
  ...
  void should_return_200_with_booking_detail_for_owner() throws Exception {
    when(bookingService.getBooking(1L, "BK-20260304-001"))
        .thenReturn(
            new BookingResponse(
                "BK-20260304-001",
                BookingStatus.CONFIRMED,
    ...
    mockMvc
        .perform(get("/api/v1/bookings/BK-20260304-001").with(authentication(userAuthentication())))
        .andExpect(status().isOk())
  ...
  void should_return_200_when_booking_cancel_is_successful() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/bookings/BK-20260304-001/cancel")
  ...
  @Test
  void should_return_200_with_quote_when_cancellation_quote_requested_for_owned_booking()
      throws Exception {
    when(cancellationService.computeQuote(1L, "BK-20260304-001", List.of(101L, 102L)))
        .thenReturn(
            new CancellationQuoteResponse(
                "BK-20260304-001", "GE_24_HOURS", 50, 150000L, 75000L, "inr"));

    mockMvc
        .perform(
            get("/api/v1/bookings/BK-20260304-001/cancellation-quote")
                .with(authentication(userAuthentication()))
  ...
  @Test
  void should_return_404_when_cancellation_quote_requested_for_non_owned_booking()
      throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/bookings/BK-UNKNOWN/cancellation-quote")
                .with(authentication(userAuthentication()))
  ...
  void should_return_200_when_cancel_items_is_successful() throws Exception {
    when(cancellationService.cancelItems(
            1L, "BK-20260304-001", List.of(101L, 102L), RefundReason.REQUESTED_BY_CUSTOMER))
        .thenReturn(
            new CancelItemsResponse(
                "BK-20260304-001",
                BookingStatus.CONFIRMED,
                List.of(101L, 102L),
                new CancelItemsResponse.ItemCancellationRefundResponse(
                    "BUYER_PARTIAL", 50, 75000L, "inr", "PENDING")));

    mockMvc
        .perform(
            post("/api/v1/bookings/BK-20260304-001/cancel-items")
                .with(authentication(userAuthentication()))
                .contentType("application/json")
  ...
  void should_return_409_when_cancel_items_not_allowed() throws Exception {
    when(cancellationService.cancelItems(
            1L, "BK-20260304-001", List.of(101L), RefundReason.REQUESTED_BY_CUSTOMER))
        .thenThrow(
            new CancellationNotAllowedException(
                "Booking is not in CONFIRMED state", Map.of("currentStatus", "PENDING")));
  ...
  void should_return_400_when_cancel_items_request_has_invalid_item_ids() throws Exception {
    when(cancellationService.cancelItems(
            1L, "BK-20260304-001", List.of(999L), RefundReason.REQUESTED_BY_CUSTOMER))
        .thenThrow(
            new InvalidCancelItemsRequestException(
