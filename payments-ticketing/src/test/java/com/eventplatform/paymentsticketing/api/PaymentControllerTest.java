...
class PaymentControllerTest {
    ...
    void should_return_200_with_client_secret_when_checkout_exists() throws Exception {
        when(paymentService.getCheckout(42L, 1L)).thenReturn(new CheckoutInitResponse(
            42L,
            "BK-20260304-001",
            "pi_123",
            "secret_123",
        ...
        mockMvc.perform(get("/api/v1/payments/checkout/42").with(authentication(userAuthentication())))
    ...
    void should_return_409_when_payment_already_exists() throws Exception {
        when(paymentService.getCheckout(42L, 1L)).thenThrow(new DuplicatePaymentException("BK-20260304-001"));
    ...
    void should_return_200_when_confirm_succeeds() throws Exception {
        when(paymentService.confirmPayment(1L, "pi_123")).thenReturn(new BookingResponse(
            "BK-20260304-001",
    ...
    void should_return_402_when_confirm_requires_action() throws Exception {
        when(paymentService.confirmPayment(1L, "pi_123")).thenThrow(new PaymentNotConfirmedException("requires_action"));
    ...
    void should_return_409_when_confirm_called_for_already_confirmed_booking() throws Exception {
        when(paymentService.confirmPayment(1L, "pi_123")).thenThrow(new DuplicatePaymentException("BK-20260304-001"));
    ...
    void should_return_200_when_failure_reported_by_frontend() throws Exception {
        mockMvc.perform(post("/api/v1/payments/failed")
                .with(authentication(userAuthentication()))
                .contentType("application/json")
                .content("{\"paymentIntentId\":\"pi_123\"}"))
            .andExpect(status().isOk())
    ...
    void should_return_404_when_failed_called_with_unknown_payment_intent() throws Exception {
        doThrow(new PaymentIntentNotFoundException("pi_missing"))
            .when(paymentService).handleFailure("pi_missing", null, "frontend_reported_failure");
