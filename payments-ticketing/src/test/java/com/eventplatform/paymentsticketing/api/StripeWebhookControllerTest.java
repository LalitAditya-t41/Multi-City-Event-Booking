...
class StripeWebhookControllerTest {
  ...
  @MockitoBean private StripeWebhookHandler stripeWebhookHandler;
  ...
  @Test
  void should_return_200_and_confirm_payment_when_payment_intent_succeeded_event_arrives()
      throws Exception {
  ...
  void should_return_200_and_handle_payment_failed_event() throws Exception {
    String payload =
        "{\"data\":{\"object\":{\"last_payment_error\":{\"code\":\"card_declined\",\"message\":\"Declined\"}}}}";
    when(stripeWebhookHandler.parse(payload, "sig"))
        .thenReturn(
            new StripeWebhookEvent("evt_2", "payment_intent.payment_failed", "pi_123", payload));
  ...
  void should_return_200_for_unknown_payment_intent_id_noop() throws Exception {
    when(stripeWebhookHandler.parse("{}", "sig"))
        .thenReturn(
            new StripeWebhookEvent("evt_6", "payment_intent.succeeded", "pi_missing", "{}"));
