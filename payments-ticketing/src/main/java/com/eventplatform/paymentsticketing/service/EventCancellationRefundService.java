...
public class EventCancellationRefundService {
    ...
    public void processEventRefunds(Long slotId, Long orgId) {
        List<Booking> confirmedBookings = bookingRepository.findBySlotIdAndStatus(slotId, BookingStatus.CONFIRMED);
        ...
        for (Booking booking : confirmedBookings) {
            ...
            try {
                transactionTemplate.executeWithoutResult(status -> {
                    EventCancellationRefundAudit audit = eventCancellationRefundAuditRepository.findBySlotIdAndBookingId(slotId, booking.getId())
                    ...
                    // FR6 stub seam: no dedicated batch Stripe call yet; reuses cancellation orchestration.
    ...
    private void ensureAuditRow(Long slotId, Long bookingId) {
        transactionTemplate.executeWithoutResult(status -> {
            eventCancellationRefundAuditRepository.findBySlotIdAndBookingId(slotId, bookingId)
                .orElseGet(() -> eventCancellationRefundAuditRepository.save(new EventCancellationRefundAudit(slotId, bookingId)));
        });
    }
}
