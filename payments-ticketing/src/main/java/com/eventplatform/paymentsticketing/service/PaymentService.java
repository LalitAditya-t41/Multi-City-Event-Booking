...
public class PaymentService {
  ...
  public CheckoutInitResponse createCheckout(CartAssembledEvent event) {
    ...
    if (existingBooking != null) {
      Payment existingPayment =
          paymentRepository.findByBookingId(existingBooking.getId()).orElse(null);
      if (existingPayment != null
        ...
        StripePaymentIntentResponse paymentIntent =
            stripePaymentService.getPaymentIntent(existingPayment.getStripePaymentIntentId());
    ...
    if (slotSummary == null) {
      throw new CartItemsFetchException(
          "Unable to resolve slot details for cart=" + event.cartId());
    }
    ...
    StripePaymentIntentRequest request =
        new StripePaymentIntentRequest(
            ...
            "Booking " + bookingRef,
            "cart-" + event.cartId(),
            Map.of("booking_ref", bookingRef, "user_id", String.valueOf(event.userId())));
  ...
  public CheckoutInitResponse getCheckout(Long cartId, Long userId) {
    Booking booking =
        bookingRepository
            .findByCartId(cartId)
            .orElseThrow(() -> new BookingNotFoundException("cart-" + cartId));
    ...
    StripePaymentIntentResponse paymentIntent =
        stripePaymentService.getPaymentIntent(payment.getStripePaymentIntentId());
    return new CheckoutInitResponse(
        cartId,
  ...
  public BookingResponse confirmPayment(Long userId, String paymentIntentId) {
    ...
    StripePaymentIntentResponse stripePaymentIntent =
        stripePaymentService.getPaymentIntent(paymentIntentId);
    ...
    for (CartItemSnapshotDto cartItem : cartItems) {
      int quantity =
          cartItem.quantity() == null || cartItem.quantity() < 1 ? 1 : cartItem.quantity();
      for (int i = 0; i < quantity; i++) {
        ...
        String encodedQrData =
            Base64.getEncoder().encodeToString(rawQrData.getBytes(StandardCharsets.UTF_8));
        ETicket eTicket =
            new ETicket(
                booking.getId(),
                item.getId(),
    ...
    afterCommitEventPublisher.publish(
        new BookingConfirmedEvent(
            booking.getId(), booking.getCartId(), seatIds, paymentIntentId, booking.getUserId()));
  ...
  public void handleFailure(String paymentIntentId, String failureCode, String failureMessage) {
    ...
    afterCommitEventPublisher.publish(
        new PaymentFailedEvent(booking.getCartId(), seatIds, booking.getUserId()));
  }

  private BookingResponse buildBookingResponse(Booking booking) {
    BookingResponse response = bookingMapper.toResponse(booking);

    Map<Long, ETicket> ticketsByBookingItemId =
        eTicketRepository.findByBookingId(booking.getId()).stream()
            .collect(
                java.util.stream.Collectors.toMap(
                    ETicket::getBookingItemId, ticket -> ticket, (a, b) -> a, HashMap::new));
        ...
        bookingItemRepository.findByBookingId(booking.getId()).stream()
            .map(
                item -> {
                  ...
                  ETicketResponse mappedTicket =
                      ticket == null
                          ? null
                          : withTicketCode(
                              eTicketMapper.toResponse(ticket),
                              booking.getBookingRef(),
                              item.getId());
  ...
  private ETicketResponse withTicketCode(
      ETicketResponse rawResponse, String bookingRef, Long bookingItemId) {
        ...
        restClient
            ...
            .onStatus(
                ...
                (request, httpResponse) -> {
                  throw new CartItemsFetchException(
                      "Unable to resolve slot timing for slotId=" + slotId);
                })
