...
public class CancellationPolicyService {
  ...
  public CancellationPolicyResponse createPolicy(
      Long adminUserId, CancellationPolicyRequest request) {
  ...
  public CancellationPolicyResponse updatePolicy(Long policyId, CancellationPolicyRequest request) {
    ...
    CancellationPolicy policy =
        cancellationPolicyRepository
            ...
            .orElseThrow(
                () ->
                    new CancellationPolicyNotFoundException(
                        "Cancellation policy not found: " + policyId));
  ...
  public RefundCalculationResult calculateRefund(Booking booking, long baseAmountInSmallestUnit) {
    ...
    CancellationPolicyTier matchedTier =
        tiers.stream()
            .filter(
                tier ->
                    tier.getHoursBeforeEvent() == null
                        || hoursUntilEvent >= tier.getHoursBeforeEvent())
            .findFirst()
    ...
    String tierLabel =
        matchedTier.getHoursBeforeEvent() == null
            ? "DEFAULT_TIER"
        ...
        .orElseGet(
            () -> {
              CancellationPolicy fallback =
                  new CancellationPolicy(null, CancellationPolicyScope.SYSTEM_DEFAULT, null);
              fallback.replaceTiers(List.of(new CancellationPolicyTier(null, 100, 1)));
              return fallback;
            });
  }

  private List<CancellationPolicyTier> toDomainTiers(
      List<CancellationPolicyRequest.TierRequest> tiers) {
    return tiers.stream()
        .map(
            tier ->
                new CancellationPolicyTier(
                    tier.hoursBeforeEvent(), tier.refundPercent(), tier.sortOrder()))
        .toList();
  }
  ...
  private void validateTiers(List<CancellationPolicyRequest.TierRequest> tiers) {
    ...
    List<CancellationPolicyRequest.TierRequest> sorted =
        tiers.stream()
    ...
    for (CancellationPolicyRequest.TierRequest tier : sorted) {
      ...
      if (tier.hoursBeforeEvent() == null) {
        ...
        if (previousHoursBeforeEvent != null
            && tier.hoursBeforeEvent() > previousHoursBeforeEvent) {
          throw new InvalidPolicyTierConfigException(
