...
public class Booking extends BaseEntity {
  ...
  public void confirm(String stripePaymentIntentId, String stripeChargeId) {
    if (status != BookingStatus.PENDING) {
      throw new BusinessRuleException(
          "Booking cannot be confirmed from state " + status, "INVALID_BOOKING_STATE");
    }
  ...
  public void markCancellationPending() {
    if (status != BookingStatus.CONFIRMED) {
      throw new BusinessRuleException(
          "Booking cannot enter cancellation pending from state " + status,
  ...
  public void cancel() {
    if (status != BookingStatus.CANCELLATION_PENDING) {
      throw new BusinessRuleException(
          "Booking cannot be cancelled from state " + status, "INVALID_BOOKING_STATE");
    }
