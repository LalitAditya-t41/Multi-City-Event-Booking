...
@Import({
  ...
  ShowSlotControllerTest.MockBeans.class
})
class ShowSlotControllerTest {
  ...
  static class MockBeans {
    @Bean
    ShowSlotService showSlotService() {
      return mock(ShowSlotService.class);
    }

    @Bean
    ShowSlotMapper showSlotMapper() {
      return mock(ShowSlotMapper.class);
    }
  ...
  @Autowired private ShowSlotService showSlotService;
  @Autowired private ShowSlotMapper showSlotMapper;
  ...
  void should_return_400_when_endTime_is_not_after_startTime() throws Exception {
    CreateShowSlotRequest request =
        new CreateShowSlotRequest(
            10L,
            "Show",
            "Desc",
  ...
  void should_return_502_when_createDraft_fails() throws Exception {
    when(showSlotService.submitSlot(eq(1L), eq(1L))).thenThrow(new EbIntegrationException("boom"));

    mockMvc
        .perform(post("/api/v1/scheduling/slots/1/submit").queryParam("orgId", "1"))
  ...
  void should_return_207_when_internal_update_succeeds_but_EB_sync_fails() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/scheduling/slots/1")
                .queryParam("orgId", "1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    objectMapper.writeValueAsString(
                        new UpdateShowSlotRequest("New", null, null, null, null, null))))
  ...
  void should_return_200_with_pricing_tiers_for_authenticated_user() throws Exception {
    ShowSlotPricingTier tier =
        new ShowSlotPricingTier(
            "General",
            new com.eventplatform.shared.common.domain.Money(BigDecimal.ZERO, "INR"),
            50,
            com.eventplatform.scheduling.domain.enums.TierType.FREE);
    ShowSlotPricingTierResponse response =
        new ShowSlotPricingTierResponse(
    ...
    mockMvc
        .perform(get("/api/v1/scheduling/slots/1/pricing-tiers"))
