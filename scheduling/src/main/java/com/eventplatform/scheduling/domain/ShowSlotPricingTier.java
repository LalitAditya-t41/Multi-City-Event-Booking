...
public class ShowSlotPricingTier extends BaseEntity {
  ...
  public ShowSlotPricingTier(String name, Money price, Integer quota, TierType tierType) {
    if (quota == null || quota <= 0) {
      throw new BusinessRuleException(
          "Pricing tier quota must be greater than 0", "INVALID_TIER_QUOTA");
    }
  ...
  public void setGroupDiscount(Integer threshold, BigDecimal percent) {
    if (threshold != null && threshold <= 0) {
      throw new BusinessRuleException(
          "Group discount threshold must be > 0", "INVALID_GROUP_THRESHOLD");
    }
    if (percent != null
        && (percent.compareTo(BigDecimal.ZERO) <= 0
            || percent.compareTo(new BigDecimal("100")) > 0)) {
      throw new BusinessRuleException(
          "Group discount percent must be between 0 and 100", "INVALID_GROUP_PERCENT");
    }
