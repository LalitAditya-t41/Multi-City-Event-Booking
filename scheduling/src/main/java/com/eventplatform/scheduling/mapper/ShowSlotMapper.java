...
public interface ShowSlotMapper {
  ...
  default ShowSlot toEntity(
      Long organizationId, Long venueId, Long cityId, CreateShowSlotRequest request) {
    return new ShowSlot(
        organizationId,
        venueId,
        cityId,
  ...
  default VenueOptionDto toVenueDto(VenueOption option) {
    return new VenueOptionDto(
        option.venueId(), option.venueName(), option.city(), option.capacity());
  }
}
