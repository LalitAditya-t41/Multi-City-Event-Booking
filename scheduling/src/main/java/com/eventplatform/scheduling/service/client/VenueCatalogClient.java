    ...
    try {
      ...
      if (ex.getStatusCode() == HttpStatus.NOT_FOUND) {
        throw new SchedulingNotFoundException(
            "Venue not found via catalog: " + venueId, "VENUE_NOT_FOUND");
      }
      throw new IntegrationException(
          "Failed to fetch venue from catalog", "CATALOG_INTEGRATION_ERROR");
    }
  }

  public List<CatalogVenueResponse> listVenuesByCity(Long cityId, Long organizationId) {
    try {
      CatalogVenueListResponse response =
          restClient
              .get()
              .uri(
                  uriBuilder ->
      ...
      throw new IntegrationException(
          "Failed to fetch venues from catalog", "CATALOG_INTEGRATION_ERROR");
    }
  }
}
