...
public class PaymentBookingLookupService {
    ...
    public PaymentBookingLookupService(@Value("${app.internal-base-url:http://localhost:8080}") String baseUrl) {
        this.restClient = RestClient.builder().baseUrl(baseUrl).build();
    }

    public BookingEligibilitySnapshot getConfirmedBookingByUserEvent(Long userId, Long eventId) {
        try {
            BookingEligibilitySnapshot response = restClient.get()
    ...
    public record BookingEligibilitySnapshot(Long bookingId, Long userId, Long eventId, Long slotId, Long orgId, String status) {
    }

    public record BookingSnapshot(Long bookingId, Long userId, Long eventId, Long slotId, Long orgId, String status) {
    }
}
