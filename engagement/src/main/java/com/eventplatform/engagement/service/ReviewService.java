...
public class ReviewService {
  ...
  public ReviewSubmitResponse submitReview(Long userId, ReviewSubmitRequest request) {
    ...
    AttendanceVerificationStatus attendanceStatus =
        attendanceVerificationService.verify(userId, request.eventId());
  ...
  public Page<ReviewResponse> listPublishedReviews(Long eventId, Pageable pageable) {
    ...
    Page<Review> page =
        reviewRepository.findByEventIdAndStatusOrderByPublishedAtDesc(
            eventId, ReviewStatus.PUBLISHED, pageable);
    return page.map(
        review ->
            reviewMapper.toReviewResponse(
                review, identityLookupService.getDisplayName(review.getUserId())));
  }
