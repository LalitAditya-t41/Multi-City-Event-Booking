...
public class AttendanceVerificationService {
  ...
  public AttendanceVerificationStatus verify(Long userId, Long eventId) {
    PaymentBookingLookupService.BookingEligibilitySnapshot booking =
        paymentBookingLookupService.getConfirmedBookingByUserEvent(userId, eventId);
    ...
    EventMetadataLookupService.EventMetadata metadata =
        eventMetadataLookupService.getEventMetadataOrNull(eventId);
    ...
    if (email == null || email.isBlank()) {
      log.warn(
          "Cannot verify Eventbrite attendance because user email missing. userId={} eventId={}",
          userId,
          eventId);
      return AttendanceVerificationStatus.ATTENDANCE_SELF_REPORTED;
    }
    ...
    try {
      ...
      List<EbAttendeeResponse> attendees =
          ebAttendeeService.getAttendeesByEvent(orgToken, metadata.ebEventId());
      EbAttendeeResponse attendee =
          attendees.stream()
      ...
      if (attendee == null) {
        log.warn(
            "Eventbrite attendee not found. userId={} eventId={} ebEventId={}",
            userId,
            eventId,
            metadata.ebEventId());
        return AttendanceVerificationStatus.ATTENDANCE_SELF_REPORTED;
      ...
      log.warn(
          "Eventbrite attendance verification unavailable due to auth failure. userId={} eventId={}",
          userId,
          eventId);
      return AttendanceVerificationStatus.ATTENDANCE_EB_UNAVAILABLE;
    } catch (Exception ex) {
      log.warn(
          "Eventbrite attendance verification unavailable. userId={} eventId={}",
          userId,
          eventId,
          ex);
      return AttendanceVerificationStatus.ATTENDANCE_EB_UNAVAILABLE;
