...
public class ReviewEligibilityService {
    ...
    public void unlockForBooking(Long bookingId, Long userId) {
        PaymentBookingLookupService.BookingSnapshot booking = paymentBookingLookupService.getBookingById(bookingId);
        if (booking == null || booking.eventId() == null) {
            log.warn("Cannot unlock review eligibility because booking lookup failed. bookingId={} userId={}", bookingId, userId);
            return;
        }
        ...
        Optional<ReviewEligibility> existing = reviewEligibilityRepository.findByUserIdAndEventId(userId, booking.eventId());
        ...
        reviewEligibilityRepository.save(new ReviewEligibility(
            userId,
            booking.eventId(),
            booking.slotId(),
            bookingId,
    ...
    public void revokeForBooking(Long bookingId) {
        Optional<ReviewEligibility> eligibilityOpt = reviewEligibilityRepository.findByBookingId(bookingId);
        ...
        Optional<Review> reviewOpt = reviewRepository.findByUserIdAndEventId(eligibility.getUserId(), eligibility.getEventId());
    ...
    public ReviewEligibility validateEligibility(Long userId, Long eventId) {
        ReviewEligibility eligibility = reviewEligibilityRepository.findByUserIdAndEventId(userId, eventId)
            .orElseThrow(() -> new ReviewNotEligibleException("You are not eligible to review this event", "REVIEW_NOT_ELIGIBLE"));
        ...
        if (eligibility.getEligibleUntil() != null && eligibility.getEligibleUntil().isBefore(Instant.now())) {
            eligibility.markExpired();
            reviewEligibilityRepository.save(eligibility);
            throw new ReviewNotEligibleException("Review submission window has closed", "REVIEW_WINDOW_CLOSED");
        }
