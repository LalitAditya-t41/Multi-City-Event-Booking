...
public class Review extends BaseEntity {
    ...
    public void markPendingModeration() {
        if (status != ReviewStatus.SUBMITTED) {
            throw new BusinessRuleException("Review cannot enter pending moderation from state " + status, "INVALID_REVIEW_STATE");
        }
    ...
    public void approve() {
        if (status != ReviewStatus.PENDING_MODERATION) {
            throw new BusinessRuleException("Review cannot be approved from state " + status, "INVALID_REVIEW_STATE");
        }
    ...
    public void publish() {
        if (status != ReviewStatus.APPROVED && status != ReviewStatus.PENDING_MODERATION) {
            throw new BusinessRuleException("Review cannot be published from state " + status, "INVALID_REVIEW_STATE");
        }
    ...
    public void reject(String rejectionReason) {
        ...
        if (status != ReviewStatus.PENDING_MODERATION) {
            throw new BusinessRuleException("Review cannot be rejected from state " + status, "INVALID_REVIEW_STATE");
        }
