...
class AdminModerationControllerTest {
    ...
    void should_return_200_for_admin_queue_when_role_admin() throws Exception {
        when(moderationService.listAdminQueue(any(), any(), any(), any()))
            .thenReturn(new PageImpl<>(List.of(
                new AdminReviewResponse(56L, 1001L, 200L, 2, "Bad", "text", ReviewStatus.PENDING_MODERATION,
                    AttendanceVerificationStatus.EB_VERIFIED, Instant.parse("2026-03-05T16:00:00Z"), 1, null)
            ), PageRequest.of(0, 20), 1));

        mockMvc.perform(get("/api/v1/admin/engagement/reviews").with(authentication(adminAuthentication())))
    ...
    void should_return_403_for_admin_queue_when_role_user() throws Exception {
        mockMvc.perform(get("/api/v1/admin/engagement/reviews").with(authentication(userAuthentication())))
    ...
    void should_return_200_when_moderate_approve_successful() throws Exception {
        when(moderationService.applyManualDecision(any(), any(), any(), any()))
            .thenReturn(new ModerationResponse(56L, ReviewStatus.PUBLISHED, Instant.parse("2026-03-05T18:10:00Z")));

        mockMvc.perform(put("/api/v1/admin/engagement/reviews/56/moderate")
    ...
    void should_return_200_when_moderate_reject_successful() throws Exception {
        when(moderationService.applyManualDecision(any(), any(), any(), any()))
            .thenReturn(new ModerationResponse(56L, ReviewStatus.REJECTED, Instant.parse("2026-03-05T18:10:00Z")));
