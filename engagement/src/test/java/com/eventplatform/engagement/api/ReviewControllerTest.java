...
class ReviewControllerTest {
  ...
  @MockitoBean private ReviewRatingSummaryService reviewRatingSummaryService;
  ...
  void should_return_201_when_submit_review_request_is_valid() throws Exception {
    when(reviewService.submitReview(any(), any()))
        .thenReturn(
            new ReviewSubmitResponse(55L, ReviewStatus.PENDING_MODERATION, "under moderation"));
    ...
    mockMvc
        .perform(
            post("/api/v1/engagement/reviews")
                ...
                .content(
                    "{\"eventId\":1001,\"rating\":4,\"title\":\"Great\",\"body\":\"Great show\"}"))
    ...
    mockMvc
        .perform(
            post("/api/v1/engagement/reviews")
                ...
                .contentType("application/json")
                .content(
                    "{\"eventId\":1001,\"rating\":0,\"title\":\"Great\",\"body\":\"Great show\"}"))
  ...
  void should_return_200_for_public_published_reviews_endpoint() throws Exception {
    when(reviewService.listPublishedReviews(any(), any()))
        .thenReturn(
            new PageImpl<>(
                List.of(
                    new ReviewResponse(
                        ...
                        Instant.parse("2026-03-05T10:10:00Z"),
                        "Jane D")),
                PageRequest.of(0, 20),
                1));

    mockMvc
        .perform(get("/api/v1/engagement/reviews/events/1001"))
  ...
  void should_apply_default_sort_published_at_desc_when_no_sort_param() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/engagement/reviews/events/1001")).andExpect(status().isOk());
    ...
    when(reviewRatingSummaryService.getSummary(1001L))
        .thenReturn(
            new ReviewSummaryResponse(
                1001L, new BigDecimal("4.2"), 142L, Map.of(5, 80L), Instant.now()));

    mockMvc
        .perform(get("/api/v1/engagement/reviews/events/1001/summary"))
        .andExpect(status().isOk())
  ...
  void should_return_200_for_my_reviews_when_authenticated() throws Exception {
    when(reviewService.listMyReviews(any(), any()))
        .thenReturn(
            new PageImpl<>(
                List.of(
                    new ReviewResponse(
    ...
    mockMvc
        .perform(get("/api/v1/engagement/reviews/me").with(authentication(userAuthentication())))
        .andExpect(status().isOk())
  ...
  void should_return_401_for_my_reviews_without_jwt() throws Exception {
    mockMvc.perform(get("/api/v1/engagement/reviews/me")).andExpect(status().isUnauthorized());
  }
