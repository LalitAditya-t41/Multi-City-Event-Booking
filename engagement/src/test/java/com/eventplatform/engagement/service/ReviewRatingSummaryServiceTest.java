...
class ReviewRatingSummaryServiceTest {
    ...
    @InjectMocks
    private ReviewRatingSummaryService service;

    @Test
    void should_return_summary_from_cache_when_cache_hit() {
        when(redisTemplate.opsForValue()).thenReturn(valueOperations);
        ReviewRatingSummary cached = new ReviewRatingSummary(1001L, new BigDecimal("4.2"), 10L, Map.of(1, 1L), Instant.now());
    ...
    void should_compute_and_cache_summary_when_cache_miss() {
        ...
        when(reviewRepository.summarizeByEventId(1001L)).thenReturn(new Object[]{
            new BigDecimal("4.25"), 20L, 1L, 2L, 3L, 4L, 10L
        });

        var response = service.getSummary(1001L);

        assertThat(response.averageRating()).isEqualTo(new BigDecimal("4.3"));
        verify(valueOperations).set(eq("engagement:review:summary:1001"), any(ReviewRatingSummary.class), eq(Duration.ofMinutes(5)));
    }

    @Test
    void should_fallback_to_db_when_redis_get_throws() {
        when(redisTemplate.opsForValue()).thenReturn(valueOperations);
        when(valueOperations.get("engagement:review:summary:1001")).thenThrow(new RuntimeException("redis down"));
        when(reviewRepository.summarizeByEventId(1001L)).thenReturn(new Object[]{
            new BigDecimal("4.0"), 5L, 1L, 1L, 1L, 1L, 1L
        });
    ...
    void should_return_result_when_redis_set_throws() {
        ...
        doThrow(new RuntimeException("set fail")).when(valueOperations)
            .set(eq("engagement:review:summary:1001"), any(ReviewRatingSummary.class), eq(Duration.ofMinutes(5)));
