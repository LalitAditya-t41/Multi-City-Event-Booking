...
public class CouponEligibilityService {
    ...
    public DiscountBreakdownResponse validateAndApply(Long userId, CouponValidateRequest request) {
        ...
        if (promotion.getScope() == PromotionScope.EVENT_SCOPED) {
            ...
            if (slot == null || slot.ebEventId() == null || !slot.ebEventId().equals(promotion.getEbEventId())) {
        ...
        if (promotion.getPerUserCap() != null
            && redemptionRepository.countByCouponIdAndUserIdAndVoidedFalse(coupon.getId(), userId) >= promotion.getPerUserCap()) {
    ...
    public DiscountCalculationResult calculateDiscount(Coupon coupon, String currency, Long cartId) {
        ...
        long discount;
        if (coupon.getPromotion().getDiscountType() == com.eventplatform.promotions.domain.enums.DiscountType.PERCENT_OFF) {
            BigDecimal value = BigDecimal.valueOf(subtotal)
                .multiply(coupon.getPromotion().getDiscountValue())
                .divide(BigDecimal.valueOf(100), 0, RoundingMode.FLOOR);
            discount = value.longValue();
        } else {
            long amountOff = coupon.getPromotion().getDiscountValue()
