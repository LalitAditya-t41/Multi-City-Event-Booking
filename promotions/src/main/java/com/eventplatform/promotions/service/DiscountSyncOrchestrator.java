...
public class DiscountSyncOrchestrator {
  ...
  public void createSyncInternal(Coupon coupon) {
    ...
    if (promotion.getScope() == PromotionScope.EVENT_SCOPED && promotion.getEbEventId() == null) {
      ...
      log.warn(
          "Skipping EB coupon sync due to null ebEventId for event-scoped promotion. couponId={}",
          coupon.getId());
      return;
    }
    ...
    try {
      ...
      var existing =
          ebDiscountSyncService.listDiscounts(orgId).stream()
      ...
      if (existing.isPresent()) {
        ...
        adoptedOrCreated =
            ebDiscountSyncService.createDiscount(orgId, payloadMapper.toRequest(coupon, promotion));
      }
  ...
  public void guardedDeleteInternal(Coupon coupon) {
    ...
    try {
      ...
      if (isDeleteBlocked(ex)) {
        coupon.markDeleteBlocked(
            coupon.getEbQuantitySoldAtLastSync() == null
                ? 1
      ...
      couponRepository.save(coupon);
      log.warn(
          "EB guarded delete failed for couponId={} status={}",
          coupon.getId(),
  ...
  public void resyncAfterPromotionUpdate(Long couponId) {
    couponRepository
        ...
        .ifPresent(this::resyncAfterPromotionUpdateInternal);
  }
