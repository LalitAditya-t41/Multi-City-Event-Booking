...
public class CouponRedemptionService {
  ...
  public void onBookingConfirmed(Long bookingId, Long cartId, Long userId) {
    ...
    CouponUsageReservation reservation =
        reservationRepository.findOpenByCartId(cartId).orElse(null);
    if (reservation == null) {
      return;
    }

    Coupon coupon =
        couponRepository.findByIdForUpdate(reservation.getCoupon().getId()).orElse(null);
    if (coupon == null) {
      reservation.release();
    ...
    if (redemptionRepository.existsByCouponIdAndBookingIdAndVoidedFalse(
        coupon.getId(), bookingId)) {
      reservation.release();
      reservationRepository.save(reservation);
      return;
    }

    DiscountCalculationResult calc =
        eligibilityService.calculateDiscount(coupon, summary.currency(), cartId);
    BigDecimal discountMajor =
        BigDecimal.valueOf(calc.discountAmountInSmallestUnit()).divide(BigDecimal.valueOf(100));

    redemptionRepository.save(
        new CouponRedemption(coupon, userId, bookingId, cartId, discountMajor, calc.currency()));
  ...
  public void onBookingCancelled(Long bookingId) {
    CouponRedemption redemption =
        redemptionRepository.findByBookingIdAndVoidedFalse(bookingId).orElse(null);
    if (redemption == null) {
      return;
    }
    ...
    if (coupon.getEbSyncStatus() == EbSyncStatus.DELETE_BLOCKED
        || coupon.getEbSyncStatus() == EbSyncStatus.EB_DELETED_EXTERNALLY) {
      log.info(
          "Skipping EB recreate for couponId={} due to syncStatus={}",
          coupon.getId(),
  ...
  public void onCartAssembled(Long cartId, Long userId) {
    ...
    Coupon coupon =
        couponRepository.findByCodeIgnoreCaseAndOrgId(couponCode, summary.orgId()).orElse(null);
    if (coupon == null) {
      return;
    }
    ...
    eventPublisher.publishEvent(
        new CouponAppliedEvent(
            cartId, couponCode, result.discountAmountInSmallestUnit(), result.currency(), userId));
    log.debug(
        "Cart assembled coupon check done cartId={} discount={}",
        cartId,
