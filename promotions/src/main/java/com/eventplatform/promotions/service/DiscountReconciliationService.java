...
public class DiscountReconciliationService {
  ...
  public void reconcileOrg(Long orgId) {
    ...
    try {
      List<EbDiscountResponse> ebDiscounts =
          ebDiscountSyncService.listDiscounts(String.valueOf(orgId));
      Map<String, EbDiscountResponse> byId =
          ebDiscounts.stream()
              .filter(d -> d.id() != null)
              .collect(Collectors.toMap(EbDiscountResponse::id, Function.identity(), (a, b) -> a));

      List<Coupon> syncedCoupons =
          couponRepository.findByOrgIdAndEbSyncStatus(orgId, EbSyncStatus.SYNCED);
      checked = syncedCoupons.size();

      for (Coupon coupon : syncedCoupons) {
        EbDiscountResponse eb =
            coupon.getEbDiscountId() == null ? null : byId.get(coupon.getEbDiscountId());
        if (eb == null) {
      ...
      for (EbDiscountResponse eb : ebDiscounts) {
        if (eb.id() != null && !internalByEbId.containsKey(eb.id())) {
          orphanRepository.save(
              new OrphanEbDiscount(eb.id(), orgId, eb.code() == null ? "UNKNOWN" : eb.code()));
          orphans++;
        }
      }
    ...
    logRepository.save(
        new DiscountReconciliationLog(orgId, checked, drifts, orphans, deleted, summary, error));
  }

  @Transactional(readOnly = true)
  public DiscountReconciliationLogResponse latestLog(Long orgId) {
    DiscountReconciliationLog log =
        logRepository
            .findTopByOrgIdOrderByRunAtDesc(orgId)
            .orElseThrow(
                () ->
                    new com.eventplatform.shared.common.exception.ResourceNotFoundException(
                        "No reconciliation run for org=" + orgId, "RECONCILIATION_LOG_NOT_FOUND"));
