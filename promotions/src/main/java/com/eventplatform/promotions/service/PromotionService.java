...
public class PromotionService {
    ...
    @Transactional
    public com.eventplatform.promotions.api.dto.response.PromotionResponse create(Long orgId, PromotionCreateRequest request) {
    ...
    public List<com.eventplatform.promotions.api.dto.response.PromotionResponse> list(Long orgId, PromotionStatus status) {
    ...
    @Transactional(readOnly = true)
    public com.eventplatform.promotions.api.dto.response.PromotionResponse get(Long orgId, Long promotionId) {
    ...
    @Transactional
    public com.eventplatform.promotions.api.dto.response.PromotionResponse update(Long orgId, Long promotionId, PromotionUpdateRequest request) {
        Promotion promotion = promotionRepository.findById(promotionId)
            .filter(p -> p.getOrgId().equals(orgId))
            .orElseThrow(() -> new PromotionNotFoundException(promotionId));

        promotion.updateWindowAndCaps(request.validFrom(), request.validUntil(), request.maxUsageLimit(), request.perUserCap());
        Promotion saved = promotionRepository.save(promotion);

        couponRepository.findByPromotionId(saved.getId()).forEach(coupon -> {
            if (coupon.getEbSyncStatus() == EbSyncStatus.SYNCED || coupon.getEbSyncStatus() == EbSyncStatus.SYNC_FAILED) {
    ...
    public void deactivate(Long orgId, Long promotionId) {
        ...
        if (!couponIds.isEmpty() && couponRedemptionRepository.existsByCouponIdInAndVoidedFalse(couponIds)) {
