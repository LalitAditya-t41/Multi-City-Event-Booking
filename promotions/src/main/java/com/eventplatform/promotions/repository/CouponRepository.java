...
public interface CouponRepository extends JpaRepository<Coupon, Long> {
  ...
  @Query(
      "select c from Coupon c where lower(c.code) = lower(:code) and c.orgId = :orgId "
          + "and c.status <> com.eventplatform.promotions.domain.enums.CouponStatus.INACTIVE")
  Optional<Coupon> findActiveOrExhaustedByCodeAndOrg(
      @Param("code") String code, @Param("orgId") Long orgId);
  ...
  @Query(
      "select c from Coupon c join c.promotion p where c.status = 'ACTIVE' and p.validUntil < :now")
