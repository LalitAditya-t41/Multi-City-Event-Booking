...
class CouponExpiryJobTest {
  ...
  @InjectMocks private CouponExpiryJob job;
  ...
  void expiryJob_should_deactivate_expired_coupons_and_release_open_reservations() {
    ...
    when(couponRepository.findByStatusAndEbSyncStatus(
            CouponStatus.ACTIVE, EbSyncStatus.SYNC_FAILED))
        .thenReturn(List.of());
  ...
  void expiryJob_should_continue_processing_remaining_coupons_when_one_fails() {
    when(couponRepository.findExpiredActiveCoupons(any()))
        .thenReturn(List.of(expiredCoupon1, expiredCoupon2, expiredCoupon3));
    when(couponRepository.findByStatusAndEbSyncStatus(
            CouponStatus.ACTIVE, EbSyncStatus.SYNC_FAILED))
        .thenReturn(List.of());
    doThrow(new RuntimeException("fail one"))
        .when(discountSyncOrchestrator)
        .guardedDelete(expiredCoupon2.getId());
