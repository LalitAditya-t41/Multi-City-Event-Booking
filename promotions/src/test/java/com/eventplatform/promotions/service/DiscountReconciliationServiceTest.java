...
class DiscountReconciliationServiceTest {
  ...
  @Mock private OrphanEbDiscountRepository orphanRepository;
  @Mock private DiscountReconciliationLogRepository logRepository;
  @Mock private EbDiscountSyncService ebDiscountSyncService;

  @InjectMocks private DiscountReconciliationService service;
  ...
  void reconciliation_should_update_internal_redemption_count_when_EB_quantity_sold_is_higher() {
    ...
    coupon.recordRedemption(100);
    when(couponRepository.findByOrgIdAndEbSyncStatus(1L, EbSyncStatus.SYNCED))
        .thenReturn(List.of(coupon));
    when(ebDiscountSyncService.listDiscounts("1"))
        .thenReturn(
            List.of(new EbDiscountResponse("eb_1", "SAVE10", "percent", 10.0, null, 3, 10, null)));
    ...
    ArgumentCaptor<DiscountReconciliationLog> logCaptor =
        ArgumentCaptor.forClass(DiscountReconciliationLog.class);
  ...
  void reconciliation_should_create_OrphanEbDiscount_when_no_internal_match_found() {
    ...
    when(ebDiscountSyncService.listDiscounts("1"))
        .thenReturn(
            List.of(
                new EbDiscountResponse("orphan_id", "ORPHAN", "percent", 10.0, null, 0, 10, null)));
  ...
  void reconciliation_should_set_DRIFT_DETECTED_when_EB_fields_differ_from_internal() {
    ...
    when(ebDiscountSyncService.listDiscounts("1"))
        .thenReturn(
            List.of(new EbDiscountResponse("eb_1", "SAVE20", "percent", 10.0, null, 0, 10, null)));
  ...
  @Test
  void
      reconciliation_should_skip_org_and_log_on_EB_list_call_failure_without_aborting_other_orgs() {
    when(couponRepository.findDistinctOrgIdByEbSyncStatus(EbSyncStatus.SYNCED))
        .thenReturn(List.of(1L, 2L));
    when(couponRepository.findByOrgIdAndEbSyncStatus(1L, EbSyncStatus.SYNCED))
        .thenReturn(List.of(coupon));
    when(couponRepository.findByOrgIdAndEbSyncStatus(2L, EbSyncStatus.SYNCED))
        .thenReturn(List.of(coupon));
    ...
    when(ebDiscountSyncService.listDiscounts("2"))
        .thenReturn(
            List.of(new EbDiscountResponse("eb_1", "SAVE10", "percent", 10.0, null, 0, 10, null)));
