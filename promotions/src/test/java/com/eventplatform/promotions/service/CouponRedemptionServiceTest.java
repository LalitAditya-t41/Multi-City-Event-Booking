...
class CouponRedemptionServiceTest {
  ...
  @Mock private CouponEligibilityService eligibilityService;
  @Mock private CartSnapshotReader cartSnapshotReader;
  @Mock private DiscountSyncOrchestrator discountSyncOrchestrator;
  ...
  void onBookingConfirmed_should_create_redemption_and_increment_count() {
    when(cartSnapshotReader.getCartSummary(500L))
        .thenReturn(
            new CartSummaryDto(500L, 1L, 10L, "SAVE10", Instant.now().plusSeconds(1800), "inr"));
    ...
    when(redemptionRepository.existsByCouponIdAndBookingIdAndVoidedFalse(any(), any()))
        .thenReturn(false);
    when(eligibilityService.calculateDiscount(coupon, "inr", 500L))
        .thenReturn(
            new DiscountCalculationResult("SAVE10", DiscountType.PERCENT_OFF, 1000L, 9000L, "inr"));
  ...
  @Test
  void
      onBookingConfirmed_should_mark_coupon_EXHAUSTED_and_trigger_guarded_delete_when_limit_reached() {
    for (int i = 0; i < 4; i++) {
    ...
    when(eligibilityService.calculateDiscount(coupon, "inr", 500L))
        .thenReturn(
            new DiscountCalculationResult("SAVE10", DiscountType.PERCENT_OFF, 500L, 0L, "inr"));

    service.onBookingConfirmed(91L, 500L, 99L);

    assertThat(coupon.getStatus())
        .isEqualTo(com.eventplatform.promotions.domain.enums.CouponStatus.EXHAUSTED);
  ...
  void onBookingConfirmed_should_be_noop_when_no_coupon_in_cart_snapshot() {
    when(cartSnapshotReader.getCartSummary(500L))
        .thenReturn(
            new CartSummaryDto(500L, 1L, 10L, null, Instant.now().plusSeconds(1800), "inr"));
  ...
  void onBookingCancelled_should_void_redemption_and_decrement_count() {
    ...
    CouponRedemption redemption =
        new CouponRedemption(coupon, 99L, 91L, 500L, new BigDecimal("10.00"), "inr");
    when(redemptionRepository.findByBookingIdAndVoidedFalse(91L))
  ...
  void onBookingCancelled_should_revert_EXHAUSTED_to_ACTIVE_and_not_recreate_on_EB() {
    ...
    assertThat(coupon.getStatus())
        .isEqualTo(com.eventplatform.promotions.domain.enums.CouponStatus.ACTIVE);
