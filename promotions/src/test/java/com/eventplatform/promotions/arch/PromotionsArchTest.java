...
class PromotionsArchTest {
  ...
  static final ArchRule
      promotions_module_should_only_call_eventbrite_via_EbDiscountSyncService_facade =
          noClasses()
              .that()
              .resideInAPackage("..promotions..")
              .should()
              .dependOnClassesThat()
              .resideInAnyPackage(
                  "..shared.eventbrite.service.Default*", "..shared.eventbrite.client..");
  ...
  void promotions_event_listeners_should_only_accept_shared_event_types() {
    ...
    for (Class<?> listener : listeners) {
      for (Method method : listener.getDeclaredMethods()) {
        ...
        org.assertj.core.api.Assertions.assertThat(method.getParameterTypes()).hasSize(1);
        org.assertj.core.api.Assertions.assertThat(allowed).contains(method.getParameterTypes()[0]);
      }
    }
  }
}
