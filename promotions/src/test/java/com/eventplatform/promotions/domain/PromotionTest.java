...
class PromotionTest {
    ...
    void deactivate_should_cascade_inactive_status_to_all_linked_coupons() {
        Promotion promotion = promotion(Instant.now().minusSeconds(3600), Instant.now().plusSeconds(3600));
        ...
        assertThat(c1.getStatus()).isEqualTo(com.eventplatform.promotions.domain.enums.CouponStatus.INACTIVE);
        assertThat(c2.getStatus()).isEqualTo(com.eventplatform.promotions.domain.enums.CouponStatus.INACTIVE);
    }

    @Test
    void isValid_should_return_false_when_validUntil_is_in_past() {
        Promotion promotion = promotion(Instant.now().minusSeconds(7200), Instant.now().minusSeconds(3600));
    ...
    void isValid_should_return_false_when_validFrom_is_in_future() {
        Promotion promotion = promotion(Instant.now().plusSeconds(3600), Instant.now().plusSeconds(7200));
